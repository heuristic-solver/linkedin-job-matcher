...
The easiest way to run the application with all dependencies (including the local Ollama LLM) is using Docker Compose.
...
2. **Launch the entire stack**
   ...
   - Start an Ollama local LLM server
   - Automatically pull the `gemma3:12b` model

3. **Access the application**
   Open your browser at `http://localhost:5003`

> [!TIP]
> **GPU Support**: If you have an NVIDIA GPU, uncomment the `deploy` section in `docker-compose.yml` to enable hardware acceleration for much faster resume analysis.
